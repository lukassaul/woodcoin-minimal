// tx/b26ef8b882fb179a62812dc3f1067b93faff87107b256384efb15a30e2190562


// Copyright (c) 2009-2010 Satoshi Nakamoto
// Copyright (c) 2009-2014 The Bitcoin developers
// Copyright (c) 2014-2019 The Woodcoin developers
...
{
     // also check to make sure it's not a token of any kind, this is ordinary wallet stuff here
    //std::map<std::string, CToken*>::const_iterator mi = tokenMap.find(label); 

        if (tokenMap.count(label)!=0)        
            if (tokenMap.at(label).isTokenOutput(txid,vout_index))    return true;
...
void CWallet::AddToTokenIfToken(const uint256 &hash, const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fFindBlock) {
    ...
    debugToken++;
    if (debugToken < 3)  printf("called addToTokenIfToken %s \n", tx.ToString().c_str());
    ...
    if (tx.GetHash().ToString().compare("b77bd00f1e7e941bccdc9b69516a3050db7c43665b85ef7448b8ac7b43814037")==0) 
        printf("We found the early txid \n");
 
    // Do a check to make sure we don't have a genesis TX here.  If we do have a genesis txid, add the output because
    // it hasn't been automatically added due to amount not being available in wallet 
    for (std::map<std::string, CToken>::const_iterator mi = tokenMap.begin(); mi != tokenMap.end(); ++mi ) {
        if (!mi->second.genesisValueSet) { 
            // lets just step through the vouts this won't happen too often (could be more eficcienet by checking txid first)
            for (int i=0; i<numOut; i++) {
                if (mi->second.genesisTxid.compare(tx.GetHash().ToString())==0 & mi->second.genesisVout==i) {
                    printf("found a non-set genesis txid and vout while scanning %s %s \n", mi->second.label.c_str(), tx.GetHash().ToString().c_str());
                    ...
                    //if (!IsToken(mi->second.genesisTxid, mi->second.genesisVout))  {
                        printf("setting a new genesis TXOut for token %s \n", mi->second.label.c_str());
                        CToken newTokenObject = mi->second;                        
                        //p_token2 = &(*mi).second;                        
                        newTokenObject.addOutput(key, tx.vout.at(i).nValue);
                        newTokenObject.setGenesisValueSet();
                        // now replace the old one in the map
                        tokenMap.at(mi->second.label)=newTokenObject;
                    //}
                }
            }  
        }  
    ...
    for (int i=0; i<numIn; i++) {
       // if (tx.vin.at(i).prevout.n > -1) {
            if (debugToken<10) printf("looping \n");
            if (tx.vin.at(i).prevout.hash.ToString().compare("b26ef8b882fb179a62812dc3f1067b93faff87107b256384efb15a30e2190562")==0) {
                printf("Found the test TX! Should be a token here..  \n");
                printf("prevout . n = %i \n ", tx.vin.at(i).prevout.n);
            }
            if (tx.GetHash().ToString().compare("b26ef8b882fb179a62812dc3f1067b93faff87107b256384efb15a30e2190562")==0) 
                printf("Found the test in main TX! \n");
            if (IsToken(tx.vin.at(i).prevout.hash.ToString() , tx.vin.at(i).prevout.n)) {
                printf("Hey wait we found a token \n");                
                tokenName = GetTokenName(tx.vin.at(i).prevout.hash.ToString(), tx.vin.at(i).prevout.n);
                std::map<std::string, CToken>::iterator mi = tokenMap.find(tokenName);
                p_token = &(*mi).second;                 
                numValidIn++;
    ...
    if (tokenName=="") return;   // no token found 
    ...
    if (amountOut==amountTokenIn  && amountOutMan==amountOut) {
       // for (std::map<std::string, CToken*>::iterator mi = tokenMap.begin(); mi != tokenMap.end(); ++mi )  { 
            ...
            for (int j=0; j<numOut; j++) {
                ...
                if (!p_token->isTokenOutput(tx.GetHash().ToString(),j)) p_token->addOutput(tx.GetHash().ToString() + std::to_string(j), tx.vout.at(j).nValue);
            } 
            if (!p_token->isTokenTx(tx.GetHash().ToString()))  p_token->addTransaction(tx.GetHash().ToString(), voutTemp);  
   ...
   // for (std::map<std::string, CToken*>::iterator mi = tokenMap.begin(); mi != tokenMap.end(); ++mi )  {        
    ...
    if (foundChange && !doubleFault) {
       ...
       // for (std::map<std::string, CToken*>::iterator mi = tokenMap.begin(); mi != tokenMap.end(); ++mi )  {
       // if (p_token->isTokenOutput(tx.vin.at(==tx.GetHash().ToString()) {
        printf("Found a change type II token transaction while scanning \n");
        std::vector<int> tokenOuts;
        for (int m=0; m<numOut; m++) {
            if (m!=realChange)   {
                if (p_token->isTokenOutput(tx.GetHash().ToString(), tx.vout.at(m).nValue)) 
                    p_token->addOutput(tx.GetHash().ToString() + std::to_string(m), tx.vout.at(m).nValue);
                tokenOuts.push_back(m);
            }
        }
        if (p_token->isTokenTx(tx.GetHash().ToString())) p_token->addTransaction(tx.GetHash().ToString(), tokenOuts);
        return;
        //}
    }
    return;
...
int64 CWallet::GetTokenBalance(std::string name) const
{
    int64 nTotal = 0;
    {
       ...
       // std::map<std::string, CToken*>::iterator mi = tokenMap.find(name);
        ...
        if (name.compare("")==0) return GetBalance();
...
void CWallet::AvailableCoins(vector<COutput>& vCoins, bool fOnlyConfirmed, std::string token, const CCoinControl *coinControl) const
{
    vCoins.clear();
    bool aToken = !(token.compare("")==0);

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                bool takeThisVout = true;                
                if (!aToken) {
                    if (IsToken(pcoin->GetHash().ToString(), pcoin->vout[i].nValue)) 
                        takeThisVout = false;
                }    
                else if (!IsToken(pcoin->GetHash().ToString(), pcoin->vout[i].nValue, token)) 
                        takeThisVout = false;
                
                if (takeThisVout && !(pcoin->IsSpent(i)) && IsMine(pcoin->vout[i]) &&
...
//        if (IsToken(pcoin->GetHash().ToString(),output.i))
...
bool CWallet::SelectCoins(int64 nTargetValue, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64& nValueRet, 
                                        std::string token, const CCoinControl* coinControl) const
{
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, token, coinControl);
...
bool CWallet::CreateTransaction(const vector<pair<CScript, int64> >& vecSend,
                                ...
                                int64& nFeeRet, std::string& strFailReason, std::string token, const CCoinControl* coinControl)
{
    int64 nValue = 0;
    int64 nTotalValue = 0;
    bool isToken = !(token.compare("")==0);
...

    {
        LOCK2(cs_main, cs_wallet);
        {
            ...
            loop
            {
                ...
                if (isToken) nTotalValue = nValue;  // we don't want to pay fee in tokens we take care of that later
                else int64 nTotalValue = nValue + nFeeRet;  // we do want to pay fee if we are sending LOG
                ...
                BOOST_FOREACH (const PAIRTYPE(CScript, int64)& s, vecSend)
                {
                    ...
                    // isDust always returns false now so we don't need this 
                    /*if (txout.IsDust())
                    {
                ...
                // choose our primary coin or token for the transaction   
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, token, coinControl))
                {
                    if (isToken) strFailReason = _("Insufficient tokens");
                    else strFailReason = _("Insufficient funds");
                    return false;
                }

                // if the transaction sends token, we need to select some regular coin for the tx fee
                set<pair<const CWalletTx*,unsigned int> > setFeeCoins;
                int64 nValueInFee = 0;
                if (isToken) {
                    if (!SelectCoins(nFeeRet, setFeeCoins, nValueInFee, "", coinControl))
                    {
                        strFailReason = _("Insufficient fee funds for token tranasction");
                        return false;
                    }
                }   
                ...
                int64 nChange = nValueIn - nTotalValue - nFeeRet;  // this is the "main" change, regular change for regular tx, token change for token tx
                if (isToken) nChange = nValueIn - nTotalValue; // no fee paid in token
                int64 feeChange = nValueInFee - nFeeRet;  // change in regular coin 
                ...
                if (feeChange > 0)
                {
                   ...
                   //     scriptChange.SetDestination(coinControl->destChange);
                    ...
                    //else
                    {
                        ...
                        // Reserve a new key pair from key pool
                        CPubKey vchPubKey;
                        assert(reservekey2.GetReservedKey(vchPubKey)); // should never fail, as we just unlocked

                        scriptChange.SetDestination(vchPubKey.GetID());
                    }

                    CTxOut newTxOut(feeChange, scriptChange);
...
bool CWallet::CreateTransaction(CScript scriptPubKey, int64 nValue,
                                CWalletTx& wtxNew, CReserveKey& reservekey, CReserveKey& reservekey2, int64& nFeeRet, 
                                std::string& strFailReason, std::string token, const CCoinControl* coinControl)
{
    vector< pair<CScript, int64> > vecSend;
    vecSend.push_back(make_pair(scriptPubKey, nValue));
    return CreateTransaction(vecSend, wtxNew, reservekey, reservekey2, nFeeRet, strFailReason, token, coinControl);
}
...
std::string CWallet::SendTokenToDestination(const CTxDestination& address, std::string label, int64 nValue, CWalletTx& wtxNew, bool fAskFee)
{
    // Check amount
    if (nValue <= 0)
        return _("Invalid amount");
    ...
    return SendMoney(scriptPubKey, nValue, wtxNew, label, fAskFee);
}
